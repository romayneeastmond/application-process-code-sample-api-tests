...
public class MyClass
{
	...
	public void Something_Interesting() 
	{
		...
		// statistics.Dump("Statistics"); // LINQPad only		
		// statistics.OrderBy(x => x.Created).ToList().Dump("Statistics Ordered By Creation Date"); // LINQPad only
		...
		if (Directory.Exists(PathToSource))
        {
            ...
            if (files.Length > 0)
            {
                foreach (var file in files)
                {
					...
					// fileInfo.Dump("File Information"); // LINQPad only
